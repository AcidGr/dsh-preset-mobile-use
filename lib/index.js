...
function screenshotScaleText(delivery) {
	...
	return (
		`Screenshot captured. Physical Display: ${delivery.displayWidth}x${delivery.displayHeight}. Delivered image: ${delivery.width}x${delivery.height} (${how}, within untouched vision budget).\n` +
		`- Outer Coordinate Rulers: The inner screen is 100% unoccluded and framed by outer rulers on all 4 borders (Top/Bottom mark physical X: 0~${delivery.displayWidth}, Left/Right mark physical Y: 0~${delivery.displayHeight}).\n` +
		`- Direct Coordinate Reading (Recommended): Visually align your target element with the tick marks and labels on the outer rulers to directly obtain physical display coordinates [x_display, y_display]. Pass them straight to mobile(action: "click", coordinate: [x_display, y_display]).\n` +
		`- Pixel Math Conversion: The inner screen content is located at [left=${delivery.rulerLeft}, top=${delivery.rulerTop}, right=${delivery.rulerLeft + delivery.innerW}, bottom=${delivery.rulerTop + delivery.innerH}]. If reading raw pixel position (x_image, y_image) from this delivered image: x_display = Math.round((x_image - ${delivery.rulerLeft}) * ${factorText(delivery.scaleX)}) and y_display = Math.round((y_image - ${delivery.rulerTop}) * ${factorText(delivery.scaleY)}).`
	);
}
	...
	try {
		...
		if (meta && meta.width > 0 && meta.height > 0) {
			...
			delivery = {
				...
				rulerTop: RULER_TOP,
				rulerRight: RULER_RIGHT,
				rulerBottom: RULER_BOTTOM,
