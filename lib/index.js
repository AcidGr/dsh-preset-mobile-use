...
export function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			text: {
				type: "string",
				description: "String payload: text to input ('type'), key name like 'BACK'/'HOME'/'ENTER' ('key'), package name or 'package/activity' ('launch_app'), or search query ('list_apps').",
			},
			duration_ms: {
				type: "integer",
		...
		async execute(args) {
			...
			switch (args.action) {
				...
				case "launch_app": {
					const raw = String(args.text || args.package || "").trim();
					if (!raw) {
						return { message: "Error: action 'launch_app' requires package name in 'text' parameter (e.g. 'com.tencent.mm' or 'com.tencent.mm/.ui.LauncherUI')." };
					}
					let pkg = raw;
					let act = args.activity || "";
					if (raw.includes("/")) {
						const parts = raw.split("/");
						pkg = parts[0].trim();
						act = parts.slice(1).join("/").trim();
					}
					try {
						...
						const actionDesc = `OK: Launched app ${pkg}${act ? "/" + act : ""}: ${res.message || "OK"}`;
