...
export function apply(ctx) {
	...
	ctx.on("session/event", (session, event) => {
		try {
			...
			if (event?.type === "user/message") {
				if (sid && activeMobileSessions.has(sid)) {
					let title = "";
					try {
						...
						if (!title) {
							title = session?.title || session?.meta?.title || "";
						}
					} catch (_) {}
					postJson("/api/task_event", {
						type: "agent_status",
	...
	ctx.on("agent/status", async (payload) => {
		...
		// Only drive mobile capsule & notifications for sessions that actively use mobile tools!
		if (currentSessionId && !activeMobileSessions.has(currentSessionId)) {
			return;
		}
