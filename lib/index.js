...
export function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			target: {
				type: "string",
				description: "Target input field: numeric node ID from dump tree (e.g. '146'). Omit or use 'focused' to type into currently focused field (dual-track strategy: 1. Prefer node ID for direct injection; 2. On failure, fallback to clicking field coordinates to focus, then type without target).",
			},
			submit: {
				type: "boolean",
